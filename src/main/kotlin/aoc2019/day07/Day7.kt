...
import aoc2019.day05.processParameterMode
...
fun findBestResult(numbers: List<Int>, phaseSettings: String, withLoop: Boolean): Int {
    ...
    for (ps in PermutationUtil.generatePermutations(phaseSettings)) {
        ...
        (ps.indices).forEach { i: Int -> phaseSettingsArr[i] = ps[i].toString().toInt() }
        val result = if (withLoop) setupLoopingAmplifiers(mutableNumbers, phaseSettingsArr) else setupAmplifiers(
            mutableNumbers,
            phaseSettingsArr
        )
        if (result > maxResult) {
...
fun setupAmplifiers(numbers: List<Int>, phaseSettings: IntArray): Int {
    var input = 0
...
fun setupLoopingAmplifiers(numbers: List<Int>, phaseSettings: IntArray): Int {
...
fun processInput(numbers: List<Int>, input1: Int, input2: Int, i: Int): LoopAmplifierOutput {
    val mutableNumbers = ArrayList(numbers)
    ...
    while (i < numbers.size) {
        ...
        val output: Output = processParameterMode(mutableNumbers, i, opCode, inputValue)
        i += output.index
    ...
    return try {
        LoopAmplifierOutput(outputBuilder.toString().toInt(), i, mutableNumbers)
    } catch (e: NumberFormatException) {
        LoopAmplifierOutput(input1, i, mutableNumbers)
    }
}
